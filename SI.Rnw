...
PMARG_COLS <- c(MDC = "#c05502", DC = "#178a68", PMDC = "#7570b3")
...
<<helperFunsMDC-GG, echo=FALSE>>=
get_expanded_unit_precision <- function(u, U, rho) {
  Q_u <- matrix(0, nrow = U + 1, ncol = U + 1)
  prec_mult <- 1 / (1 - rho^2)
  q_base <- matrix(c(1, -rho, -rho, 1), nrow = 2) * prec_mult

  idx <- u + 1
  Q_u[1, 1]     <- q_base[1, 1]
  Q_u[1, idx]   <- q_base[1, 2]
  Q_u[idx, 1]   <- q_base[2, 1]
  Q_u[idx, idx] <- q_base[2, 2]

  return(Q_u)
}

get_boundary <- function(U) { 2 / (sqrt(U) * (1 + 1/U)) }

likeMat_type1 <- function(data, job, U, rho, ...) {
  unit_precisions <- lapply(1:U, function(u) get_expanded_unit_precision(u, U, rho))
  Lambda_tot <- Reduce('+', unit_precisions)

  # For recording, we'll track the shared parameter and unit 1
  list(
    U = U,
    unit_precisions = unit_precisions,
    Lambda_tot = Lambda_tot,
    target_unit = floor((U + 1) / 2) # We will record psi_u
  )
}


algo_MDC <- function(data, job, instance, M = 1000, ...) {
  U <- instance$U
  unit_Qs <- instance$unit_precisions
  target_idx <- instance$target_unit + 1

  # State vectors (No need for full matrices)
  mu_curr <- rep(5, U + 1)
  p_diag  <- rep(2/3, U + 1) # Diagonal precision values, 1.5 variance.
  ...
  for (m in 1:M) {
    for (u in 1:U) {
      idx <- c(1, u + 1) # Indices for phi and psi_u

      # Extract the active 2x2 data precision block
      q_data <- unit_Qs[[u]][idx, idx]

      # 1. 2x2 Posterior Precision
      p_sub <- p_diag[idx]
      Q_sub <- diag(p_sub) + q_data

      # 2. 2x2 Posterior Covariance (O(1) fast inversion)
      Sigma_sub <- solve(Q_sub)

      # 3. Update the Means for just these two parameters
      mu_curr[idx] <- as.numeric(Sigma_sub %*% (p_sub * mu_curr[idx]))

      # 4. MARGINALIZE: Extract new diagonal precisions
      p_diag[idx] <- 1 / diag(Sigma_sub)
    }
    res_phi[m] <- mu_curr[1]
    res_psi[m] <- mu_curr[target_idx]
  }
  return(data.frame(iter = 1:M, phi = res_phi, psi = res_psi, Algo = "MDC"))
}
@
...
<<ComputeMDCGauss1>>=
my_inst <- likeMat_type1(data = NULL, job = NULL, U = 100, rho = 0.9)

longMDC <- pomp::bake(
  "GaussianFailure/results/longMDC.rds", {
  algo_MDC(NULL, NULL, my_inst, M = 20000) |>
    dplyr::bind_rows(data.frame(iter = 0, phi = 5, psi = 5, Algo = 'MDC'))
  }
)

gg_MDC1 <- ggplot(longMDC, aes(x = iter)) +
  geom_line(aes(y = phi, col = 'Shared', linetype = 'Shared')) +
  geom_line(aes(y = psi, col = 'Unit-Specific', linetype = 'Unit-Specific')) +  # uncomment to see unit-specific parameters
  ylab("Parameter Value") +
  xlab("Iteration (m)") +
  ggtitle("Parameter value from initial conditions") + 
   scale_color_manual(
    name = "Parameter Type", 
    values = c(Shared = 'black', `Unit-Specific` = 'red') # Change colors as needed
  ) +
  scale_linetype_manual(
    name = "Parameter Type",
    values = c(Shared = 'solid', `Unit-Specific` = 'dashed')
  ) + 
  theme(legend.position = 'none')

# gg_MDC2 <- ggplot(longMDC %>% filter(iter > 5000), aes(x = iter)) +
#   geom_line(aes(y = phi)) +
#   ylab("(log)-Shared Parameter Value") +
#   xlab("Iteration (m)") +
#   ggtitle("Parameter after initial 5000 iterations")

gg_MDC2 <- ggplot(longMDC, aes(x = log10(iter))) +
  geom_line(aes(y = log10(phi), col = 'Shared', linetype = 'Shared')) +
  geom_line(aes(y = log10(psi), col = 'Unit-Specific', linetype = 'Unit-Specific')) +
  ylab(TeX("$\\log_{10}$ Parameter error")) +
  xlab(TeX("$\\log_{10}$ Iteration (m)")) +
  ggtitle("Parameter value from initial conditions") + 
  scale_color_manual(
    name = "Parameter Type", 
    values = c(Shared = 'black', `Unit-Specific` = 'red') # Change colors as needed
...
my_lm <- lm(log(phi)~log(iter), data = longMDC |> dplyr::filter(iter > 100))
...
Fitting a linear model to the equation $\log(\phi_m) = \log(A) - c \log(m)$ confirms this behavior, yielding an $R^2 > \Sexpr{floor(my_sum$r.squared*10000) / 10000}$.
In fact, this model suggests that the asymptotic rate of convergence to the MLE is $\mathcal{O}\big(m^{-(1-|\rho|)}\big)$ for this particular simulation design, as the estimated decay coefficient was $c = \Sexpr{abs(my_sum$coefficients[2])} \approx 0.1 = 1-|\rho|$.
...
\begin{figure}[ht]
<<fig-GGlargeRho, fig.height=2.8>>=
cowplot::plot_grid(gg_MDC1, gg_MDC2, nrow = 1, rel_widths = c(1, 1.4))
@
...
\begin{figure}[ht]
<<fig-GGlargeRho2, fig.height=5>>=
GaussianFailure <- readRDS("GaussianFailure/results/GaussianFailure.rds")

GaussLabs <- as_labeller(
  c(
    `4`   = "U == 4",
    `10`  = "U == 10",
    `50`  = "U == 50",
  ...
  default = label_parsed # 2. Tell ggplot to evaluate these as math expressions
)
GaussianFailure %>%
  filter(problem == 'type1') %>%
  mutate(Algo = factor(Algo, levels = c("MDC", "DC", "PMDC"))) %>%
  ggplot(aes(x = iter, y = log(phi), col = Algo, group = Algo)) +
  geom_line() +
  facet_grid(rho~U, scales = 'free', labeller = GaussLabs) +
  scale_color_manual(
    name = 'Algorithm',
    values = PMARG_COLS
  ) +
...
\begin{figure}[ht]
<<fig-GGlargeRho3, fig.height=3.5>>=
GG2_1 <- GaussianFailure %>%
  filter(problem == 'type2') %>%
  mutate(Algo = factor(Algo, levels = c("MDC", "DC", "PMDC"))) %>%
  ggplot(aes(x = iter, y = log(abs(phi)), col = Algo, group = Algo)) +
  geom_line() +
  facet_wrap(~U, scales = 'free', labeller = GaussLabs, nrow = 1) +
  scale_color_manual(
    name = 'Algorithm',
    values = PMARG_COLS
  ) +
  ylab(TeX("Log-Absolute $\\phi$")) +
  theme(axis.title.x = element_blank(), legend.position = 'none') +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())

GG2_2 <- GaussianFailure %>%
  filter(problem == 'type2') %>%
  mutate(Algo = factor(Algo, levels = c("MDC", "DC", "PMDC"))) %>%
  ggplot(aes(x = iter, y = log(abs(psi)), col = Algo, group = Algo)) +
  geom_line() +
  facet_wrap(~U, scales = 'free', labeller = GaussLabs, nrow = 1) +
  scale_color_manual(
    name = 'Algorithm',
  ...
  ylab(TeX("Log-absolute $\\psi_u$")) +
  xlab("Iteration")

GG2_legend <- cowplot::get_legend(GG2_2)
GG2_2 <- GG2_2 + theme(legend.position = 'none')

tmp_fig <- cowplot::plot_grid(GG2_1, GG2_2, nrow = 2, align = 'v', rel_heights = c(1, 1.2))
cowplot::plot_grid(tmp_fig, GG2_legend, rel_widths = c(3, 0.4))
@
...
\begin{figure}[ht]
<<correlationFig, fig.height=2.75, fig.width=4>>=
CI_results <- readRDS("data/Gompertz-CI-2026-06-17-RL3.rds")
...
CI_results %>% 
  ...
  ggplot(aes(x = r, y = `tau[unit1]`)) + 
  geom_point() + 
  theme_bw() + 
  xlab(TeX('$\\hat{r}$')) + 
  ylab(TeX('$\\hat{\\tau}_{1}$'))
@
\caption[Correlation Gompertz model near MLE]{\label{fig:ridge}\new{Estimated values of $r$ and $\tau_1$, based on the profile likelihood evaluation of Figure~\ref{fig:mcap}. The estimated correlation between these parameter estimates is \Sexpr{round(cor(r_vals, tau_vals), 3)}, which violates the correlation condition of Theorem~\ref{theorem:GG}, yet the MPIF algorithm still reliably obtains the MLE for this model.}}
\end{figure}
...
<<loadGompertzResults, eval=FALSE>>=
...
ll_50_diff <- (KF_max %>% filter(U == 50) %>% pull(ll_max)) - (MPIF_vals %>% filter(U == 50) %>% pull(max))

ll_100_diff <- (KF_max %>% filter(U == 100) %>% pull(ll_max)) - (MPIF_vals %>% filter(U == 100) %>% pull(max))
@
...
% However, for $U = 50$ and $U = 100$, the algorithm falls short of the true maximum by $\Sexpr{round(ll_50_diff, 2)}$ and $\Sexpr{round(ll_100_diff, 2)}$ log-likelihood units, respectively (roughly $\Sexpr{round(ll_50_diff / 50, 2)}$ and $\Sexpr{round(ll_100_diff / 100, 2)}$ per unit). While the likelihood continues to climbs steadily after 50 iterations, closing this gap requires increasing $M$ to values larger than required for $U \leq 15$.}
% 
...
<<loadTraces>>=
mpif_out <- readRDS("data/traceMPIF.rds")

all_traces <- mpif_out[[1]] |> traces() |> as.data.frame() |> mutate(.id = 1, iteration = row_number())

for (i in 2:length(mpif_out)) {
  all_traces <- bind_rows(
    all_traces, mpif_out[[i]] |> traces() |> as.data.frame() |> mutate(.id = i, iteration = row_number())
  )
}
rownames(all_traces) <- 1:nrow(all_traces)
@

\begin{figure}[ht]
<<traceFigure, fig.height=3.75, fig.width=8>>=
gg_trace1 <- all_traces |>
  pivot_longer(
    cols = -c(.id, iteration)
  ) |>
  filter(name %in% c('loglik', 'sigma', 'r', 'tau[unit30]')) %>% 
  # filter(.L1 %in% best_runs$id) %>%
  # filter(iteration > 10) %>%
  ggplot(aes(x = iteration, y = value, group = .id, color = factor(.id))) +
  geom_line() +
  guides(color = "none") +
  facet_wrap(~name, scales = "free_y", labeller = as_labeller(
    c(
      loglik = "'Log-likelihood'",
      r = TeX("$r$", output = "character"),
      sigma = TeX("$\\sigma$", output = "character"),
      `tau[unit30]` = TeX("$\\tau_{30}$", output = "character")
    ),
...
cowplot::plot_grid(gg_trace1, gg_trace2, align = 'v', nrow = 2, labels = "AUTO", rel_heights = c(1, 1.1))
@
...
<<loadCI, echo=FALSE>>=
CI_results <- readRDS("data/Gompertz-CI-2026-06-17-RL3.rds")
g50_mle <- readRDS("data/Gompterz50-mle.rds")

r_prof <- CI_results %>% 
  filter(i <= n() / 2) %>% 
  group_by(r) %>% 
  # summarize(max_ll = max(logLik))  # If exact likelihood is desired rather than estimated.
  summarize(max_ll = max(pf_ll))

r_ci <- pomp::mcap(logLik = r_prof$max_ll, parameter = r_prof$r)

sigma_prof <- CI_results %>% 
  filter(i > n() / 2) %>% 
  group_by(sigma) %>% 
  summarize(max_ll = max(pf_ll))

sigma_ci <- pomp::mcap(logLik = sigma_prof$max_ll, parameter = sigma_prof$sigma)

gg_r <- r_prof %>% 
  ggplot(aes(x = r, y = max_ll)) + 
  geom_point() + 
  geom_line(data = r_ci$fit, aes(x = parameter, y = smoothed)) +
  # geom_line(data = r_ci$fit, aes(x = parameter, y = quadratic), col = 'red') +
  # geom_smooth() +
  theme_bw() + 
  geom_vline(xintercept = r_ci$ci, linetype='longdash') +
  xlab(TeX('$r$')) + 
  ylab("log-likelihood") + 
  scale_y_continuous(limits = c(1100, 1180)) +
  geom_hline(yintercept = 1177.978) + 
  geom_vline(xintercept = g50_mle$par['r'], col = 'red') + 
  geom_vline(xintercept = r_ci$mle, linetype = 'dotted')

gg_sigma <- sigma_prof %>% 
  filter(sigma > 0.06, sigma < 0.15) %>% 
  ggplot(aes(x = sigma, y = max_ll)) + 
  geom_point() + 
  geom_line(data = sigma_ci$fit %>% filter(parameter > 0.06, parameter < 0.15), aes(x = parameter, y = smoothed)) +
  # geom_line(data = sigma_ci$fit, aes(x = parameter, y = quadratic), col = 'red') +
  # geom_smooth() +
  theme_bw() + 
  geom_vline(xintercept = sigma_ci$ci, linetype='dashed') + 
  xlab(TeX('$\\sigma$')) + 
  ylab("log-likelihood") + 
  scale_y_continuous(limits = c(1100, 1180)) +
  geom_hline(yintercept = 1177.978) + 
  geom_vline(xintercept = g50_mle$par['sigma'], col = 'red') + 
  geom_vline(xintercept = sigma_ci$mle, linetype = 'dotted')
@


\begin{figure}[ht]
<<CIfigure, fig.height=2.5, fig.width=6>>=
cowplot::plot_grid(gg_r, gg_sigma)
@
